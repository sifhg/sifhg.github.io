...
function type(element) {
    ...
    for(const CHILD of element.children) {
        const ELEMENT_DATA = [CHILD, CHILD.innerText];
        textElements.push(ELEMENT_DATA);
        CHILD.innerText = "";
        CHILD.style.display = "block";
    }
    updateElements(textElements)
    return textElements;
}

function updateElements(textElements) {
    let theTextElements = textElements;

    function displayNextText() {
        if (theTextElements.length > 0) {
            const [element, text] = theTextElements[0];
            updateText(element, text);
            theTextElements = theTextElements.slice(1);
            setTimeout(displayNextText, text.length * 1000 / 64);
        }
    }
...
function updateText(element, text) {
    ...
    const INTERVAL_ID = setInterval(() => {
        if(theText.length > 0) {
            element.innerText += theText[0] === ' ' ? '\u00A0' : theText[0];
...
SYSTEM_DIAGNOSTICS_BUTTON.addEventListener("click", (event) => {
    ...
    type(SYSTEM_DIAGNOSTICS_WINDOW);
})
